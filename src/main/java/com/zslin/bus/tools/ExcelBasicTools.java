...
//        Workbook wb = null;
        try {
            ...
            for(int i=beginLine;i<=s.getLastRowNum();i++) {
                ...
                } else if("buildPersonal0822".equals(methodName)) {
                    p = buildPersonal0822(row, isStr);
                }
                if(p!=null) {
                    result.add(p);
                }
...
//            e.printStackTrace();
        ...
        res = res == null?null:res.replaceAll(" ", ""); //去空格
        return res;
    }
     ...
     * @return
     */
    public static Personal buildPersonal0822(Row row, boolean isStr) {
        Personal p = new Personal();
        int index = 0;
        for(Cell cell : row) {
            String val = getBaseCellValue(cell, isStr);
...
//                    p.setGbldlqk(val); break;
                ...
                case 18: //是否享受资助
                    p.setSfxszz(val); break;
                case 19: //资助项目
//                    p.setZzxmmc(PersonalTools.buildZzxmmc(dataList, val)); //资助项目名称
                    p.setZzxm(val); break;
                case 20: //资助金额
                    p.setZzje(buildFloat(val)); break;
                case 21: //健康状况
